...
import { obterTaxaCampanha, verificarRegrasCampanha, calcularDescontoPrejuizo } from '../utils/taxasCampanha';
...
export function calcularTaxaPolitica({
  ...
  // elegibilidade final - NÃO deve depender de forcarCampanha

  const elegivelCampanha = elegivelCampanhaBase;
  ...
  if (usarAdicionarGarantia) {
    ...
    // NOVA REGRA: se adicionarGarantiaReal for true, considera como REAL sem validar valor

    reforco = 'REAL';

  } else if (possuiGarantiaRealOrigem) {
  ...
  // 4.5) Verificar regras de campanha (prejuízo vs normal)

  const regrasCampanha = verificarRegrasCampanha(contratos);

  

  // Se houver bloqueio por mistura de contratos, retorna taxa original sem campanha

  if (regrasCampanha.motivoBloqueio) {

    alertas.push(regrasCampanha.motivoBloqueio);
    ...
    return {
      ...
      debug: {
        ...
        coberturaGarantiaRelMaiorDevido: maiorValorDevido > 0 ? (valorGarantiaNum / maiorValorDevido) : 0,

        pisoMinimoDesconto: TAXA_MIN_DESCONTO,

        taxaPoliticaAposPiso: null,
  ...
  // Se as regras de campanha permitem e usuário ativou manualmente, aplica campanha

  // mesmo que elegibilidade base (critério de atraso) não seja atendida

  const permiteAplicarCampanha = forcarCampanha && 

                                regrasCampanha.tipoCampanhaPermitida !== null && 

                                !regrasCampanha.motivoBloqueio;



  if (permiteAplicarCampanha) {

    // Verificar se é campanha de prejuízo e se possui entrada obrigatória

    if (regrasCampanha.tipoCampanhaPermitida === 'prejuizo') {

      if (entradaPercentual < 10) {

        alertas.push('Campanha de Prejuízo exige entrada mínima de 10%. Aplicando regras gerais da Política de Repactuação.');

        // Não aplica campanha de prejuízo - usa taxa original

        taxaPoliticaBruta = null;
        ...
        taxaPoliticaBruta = obterTaxaCampanha({
          ...
          tipoCampanha: 'prejuizo'

        });

        

        // Adicionar informação sobre desconto máximo

        const descontoMaximo = calcularDescontoPrejuizo(entradaPercentual);

        if (descontoMaximo) {

          alertas.push(`Campanha de Prejuízo: desconto máximo de ${descontoMaximo}% na atualização para esta faixa de entrada.`);
